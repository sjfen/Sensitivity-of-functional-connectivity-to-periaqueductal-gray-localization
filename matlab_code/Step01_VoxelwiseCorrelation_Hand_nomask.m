%create voxelwise correlation to HAND trace

function AnalysisOutput = Step01_VoxelwiseCorrelation_Hand_nomask(AnalysisInput)
...
try
    ...
    for i = 1:NumPart
        ...
        if exist(pb04file,'file')==2 & exist(errtsfile,'file')==2
            ...
            %extract PAG signal
              TimeSeries = ['3dmaskave -mask /Users/amplmember/GoogleDrive/2017_MAPP_Sonja_PAG/transformations_healthycontrol/'  id_name ...
                  '/follow_ROI_traced+tlrc. -q /Volumes/AmplStorage2/Experiments/preproc/' id_name '/SUMA/' id_name '.rest.results/errts.' id_name ...
                  '.rest.anaticor+tlrc.BRIK > ' temppath 'TimeseriesSignal.1D'];
            ...
            GenCorr =  ['3dDeconvolve -input /Volumes/AmplStorage2/Experiments/preproc/' id_name '/SUMA/' id_name '.rest.results/errts.' id_name ...
                ...
                'TimeseriesSignal.1D -tout -rout -fitts ' temppath 'fit_subj01 -bucket CorrHand_' id_name];
            
            %get correlation r values
            GetRval = ['3dcalc -a CorrHand_' id_name '+tlrc.[Stim#1_R^2] -b CorrHand_' id_name '+tlrc.[Stim#1#0_Coef] -prefix rcoefHand_' id_name ' -expr "ispositive(b)*sqrt(a)-isnegative(b)*sqrt(a)"'];
            
            %convert correlation coefficients to Gaussian
            ConvGaus = ['3dcalc -a rcoefHand_' id_name '+tlrc. -expr ''atanh(a)'' -prefix zcoefHand_' id_name];
