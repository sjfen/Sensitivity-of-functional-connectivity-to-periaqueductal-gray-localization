%create voxelwise correlation to TRAD trace

function AnalysisOutput = Step01_VoxelwiseCorrelation_Wei_nomask(AnalysisInput)
...
try
    ...
    for i = 1:NumPart
        ...
        if exist(pb04file,'file')==2 & exist(errtsfile,'file')==2
            ...
            TimeSeries = ['3dmaskave -nball 4 -26 -14 3 -q /Volumes/AmplStorage2/Experiments/preproc/' id_name ...
            ...
            GenCorr =  ['3dDeconvolve -input /Volumes/AmplStorage2/Experiments/preproc/' id_name '/SUMA/' id_name '.rest.results/errts.' id_name ...
                ...
                'TimeseriesSignal.1D -tout -rout -fitts ' temppath 'fit_subj01 -bucket CorrWei_' id_name];
            
            %get correlation r values
            GetRval = ['3dcalc -a CorrWei_' id_name '+tlrc.[Stim#1_R^2] -b CorrWei_' id_name '+tlrc.[Stim#1#0_Coef] -prefix rcoefWei_' id_name ' -expr "ispositive(b)*sqrt(a)-isnegative(b)*sqrt(a)"'];
            
            %convert correlation coefficients to Gaussian
            ConvGaus = ['3dcalc -a rcoefWei_' id_name '+tlrc. -expr ''atanh(a)'' -prefix zcoefWei_' id_name];
